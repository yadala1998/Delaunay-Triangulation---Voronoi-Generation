...
class Voronoi:
    ...
    def addPoint(self, p):
        ...
        bad_triangles = []
        for T in self.triangles:
            # Choose one method: inCircleRobust(T, p) or inCircle(T, p)
    ...
    def getEdges(self):
        edges = []
        triangles = self.getTriangulation()
        for i in range(0,len(triangles)):
            if not isinstance(triangles[i], list):
                edge1 = triangles[i][0].toList(), triangles[i][0]                #sorted([triangles[i][0], triangles[i][1]])
                edge2 = sorted([triangles[i][1], triangles[i][2]])
                edge3 = sorted([triangles[i][2], triangles[i][0]])
...
if __name__ == '__main__':
    ...
    for curr_point in point_file:
        ...
        xyPoints.append([float(row[0]), float(row[1])])
    ...
    center = np.mean(xyPoints, axis=0)
    dt = Voronoi(center, 50 * radius)
